jQuery(function ($) {
  ...
  var init_form_video_field = function () {
    ...
    parseYoutubeSrc = function (src) {
      ...
      var regId = /^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=|shorts\/)([^#\&\?]*).*/;
  ...
  $('.ywcfav_import_button').on('click', function (e) {
    e.preventDefault();
    var t = $(this);
    $.ajax({
      type: 'POST',
      url: ywcfav.admin_url,
      data: {
        action: ywcfav.actions.import_video_from_free,
      },
      dataType: 'json',
      beforeSend: function () {
        t.addClass('yith-disabled');
      },
      complete: function () {
        t.removeClass('yith-disabled');
      },
      success: function (response) {
        yith.ui.modal(
          {
